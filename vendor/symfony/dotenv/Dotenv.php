...
final class Dotenv
{
    ...
    public function bootEnv(
        ...
        $debug =
            $_SERVER[$k] ??
            !\in_array($_SERVER[$this->envKey], $this->prodEnvs, true);
        $_SERVER[$k] = $_ENV[$k] =
            (int) $debug ||
            (!\is_bool($debug) && filter_var($debug, \FILTER_VALIDATE_BOOLEAN))
                ? "1"
                : "0";
    }
    ...
    public function populate(
        ...
        if ($updateLoadedVars) {
            ...
            $_ENV["SYMFONY_DOTENV_VARS"] = $_SERVER[
                "SYMFONY_DOTENV_VARS"
            ] = $loadedVars;
    ...
    private function lexVarname(): string
    {
        ...
        if (
            ...
            throw $this->createFormatException(
    ...
    private function lexValue(): string
    {
        ...
        if (
            ...
            throw $this->createFormatException(
                "Whitespace are not supported before the value"
            );
        }
        ...
        do {
            if ("'" === $this->data[$this->cursor]) {
                ...
                $resolvedValue = $value;
                $resolvedValue = $this->resolveVariables(
                    $resolvedValue,
                    $loadedVars
                );
                $resolvedValue = $this->resolveCommands(
                    $resolvedValue,
                    $loadedVars
                );
    ...
    private function resolveCommands(string $value, array $loadedVars): string
    {
        ...
        return preg_replace_callback(
            $regex,
            function ($matches) use ($loadedVars) {
                if ("\\" === $matches[1]) {
                ...
                if ("\\" === \DIRECTORY_SEPARATOR) {
                    throw new \LogicException(
                        "Resolving commands is not supported on Windows."
                    );
                }

                if (!class_exists(Process::class)) {
                    throw new \LogicException(
                        "Resolving commands requires the Symfony Process component."
                    );
                }

                $process = method_exists(Process::class, "fromShellCommandline")
                    ? Process::fromShellCommandline("echo " . $matches[0])
                    : new Process("echo " . $matches[0]);

                if (
                    !method_exists(Process::class, "fromShellCommandline") &&
                    method_exists(Process::class, "inheritEnvironmentVariables")
                ) {
        ...
        $value = preg_replace_callback(
            ...
            function ($matches) use ($loadedVars) {
                ...
                if (
                    ...
                    !isset($matches["closing_brace"])
                ) {
                    throw $this->createFormatException(
                        "Unclosed braces on variable expansion"
                    );
                }
                ...
                if (
                    "" === $value &&
                    isset($matches["default_value"]) &&
                    "" !== $matches["default_value"]
                ) {
                    $unsupportedChars = strpbrk(
                        $matches["default_value"],
                        '\'"{$'
                    );
    ...
    private function doLoad(bool $overrideExistingVars, array $paths): void
    {
        foreach ($paths as $path) {
            ...
            $this->populate(
                $this->parse(file_get_contents($path), $path),
                $overrideExistingVars
            );
        }
    }
