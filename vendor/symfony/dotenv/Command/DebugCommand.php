...
final class DebugCommand extends Command
{
    ...
    protected function execute(
        ...
        if (\in_array(".env.local.php", $availableFiles, true)) {
            $io->warning(
                "Due to existing dump file (.env.local.php) all other dotenv files are skipped."
            );
        }

        if (
            is_file($this->getFilePath(".env")) &&
            is_file($this->getFilePath(".env.dist"))
        ) {
            $io->warning(
                "The file .env.dist gets skipped due to the existence of .env."
            );
        }

        $io->section("Scanned Files (in descending priority)");
        $io->listing(
            array_map(static function (string $envFile) use ($availableFiles) {
    ...
    private function getVariables(array $envFiles): array
    {
        ...
        foreach ($vars as $var) {
            ...
            foreach ($envFiles as $envFile) {
                ...
                $shortenedVar = $this->getHelper("formatter")->truncate(
                    $varString,
                    30
                );
                $varDetails[] =
                    $varString === $realValue
                        ? "<fg=green>" . $shortenedVar . "</>"
                        : $shortenedVar;
            }
